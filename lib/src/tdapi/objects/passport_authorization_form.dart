...
class PassportAuthorizationForm extends TdObject {
  ...
  PassportAuthorizationForm.fromJson(Map<String, dynamic> json)  {
    this.id = json['id'];
    this.requiredElements = List<PassportRequiredElement>.from((json['required_elements'] ?? []).map((item) => PassportRequiredElement.fromJson(item ?? <String, dynamic>{})).toList());
  ...
  Map<String, dynamic> toJson() {
    return {
      "@type": CONSTRUCTOR,
      "id": this.id,
      "required_elements": this.requiredElements.map((i) => i.toJson()).toList(),
